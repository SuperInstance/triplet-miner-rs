...
pub fn evaluate_triplets(ap_sims: &[f64], an_sims: &[f64]) -> TripletMetrics {
    ...
    if n == 0 { return TripletMetrics { count: 0, avg_anchor_positive_sim: 0.0, avg_anchor_negative_sim: 0.0, separation: 0.0 }; }
    let avg_ap: f64 = ap_sims.iter().take(n).sum::<f64>() / n as f64;
    let avg_an: f64 = an_sims.iter().take(n).sum::<f64>() / n as f64;
    TripletMetrics { count: n, avg_anchor_positive_sim: avg_ap, avg_anchor_negative_sim: avg_an, separation: avg_ap - avg_an }
}
